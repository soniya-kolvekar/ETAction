...
- **Admin & Controller Portal**:
  ...
  - `/admin/network`: Ground-truth Section Admin Command Center ("AreWeThereYet") designed around the **Smart India Hackathon (SIH 2026)** presentation identity. Features a warm cream backdrop (`#FAF7EE`), crisp high-contrast dark slate borders (`#1E293B`), SIH vibrant orange (`#FF8A00`) and cerulean blue (`#0284C7`) accents, 3-scenario cyclic automation directly mirroring Slide 1 ("How we work?"), custom station nodes, smoothstep track edges with multi-train color discrimination, dual speed derivation meters (axles/sec vs block transit), and an authentic **Section Control & Interlocking Telemetry** panel monitoring 3-aspect ABS signals, point machine routes, headway buffers, 25 kV AC OHE traction voltage, CWR rail temperature, and dual-channel axle counter hot-standby redundancy.
