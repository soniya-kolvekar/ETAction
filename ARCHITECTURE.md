...
- **Admin & Controller Portal**:
  ...
  - `/admin/network`: Ground-truth Section Admin Command Center ("AreWeThereYet?") designed with an executive, sleek operational aesthetic. Features a warm cream backdrop (`#FAF7EE`), crisp high-contrast dark slate borders (`#1E293B`), refined orange (`#FF8A00`) and cerulean blue (`#0284C7`) accents, 4 balanced top KPI metric cards (Monitored Blocks, Active Trains, Network Velocity, Caution Orders) with tactile micro-interactions, 2D React Flow network topology with memoized node/edge definitions, an integrated **Route Station ETAs & Progression Timeline** panel on the right displaying a continuous vertical route line connecting intermediate cities/stations with scheduled times, dynamic ETAs, and `+-time` delay badges (interactively selectable per train), dual speed derivation meters (axles/sec vs block transit), and an authentic **Section Control & Interlocking Telemetry** panel monitoring 3-aspect ABS signals, point machine routes, headway buffers, 25 kV AC OHE traction voltage, CWR rail temperature, and dual-channel axle counter hot-standby redundancy.
